...
def transform(
    ...
    if not transform_type:
        ...
        th = (np.max(powers) + cp.min(powers))/2.0
        for i in range(10):
...
def stft(
    ...
    if len(data) % frame_shift == 0:
        return np.array(librosa.stft(data, n_fft=fft_size, win_length=frame_size,
...
def get_labeledSTFT(
    ...
    n_speakers = np.array([n_speakers]).astype(np.int32)


    if use_speaker_id:
        return Y, T, S
    else:
        return n_speakers, Y, T
